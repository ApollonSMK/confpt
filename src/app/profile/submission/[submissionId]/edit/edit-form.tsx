...
export function EditSubmissionForm({ submission, confrarias, discoveryTypes }: EditSubmissionFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      id: submission.id,
      title: submission.discovery_title || '',
      editorial: submission.editorial || '',
      district: submission.district as any,
      municipality: submission.municipality || undefined,
      type_id: String(submission.type),
      confrariaId: String(submission.confraria_id || 'null'),
  ...
  useEffect(() => {
      if (submission.district && portugalDistricts[submission.district as keyof typeof portugalDistricts]) {
          setMunicipalities(portugalDistricts[submission.district as keyof typeof portugalDistricts]);
      }
  }, [submission.district]);
  ...
  async function onSubmit(values: FormValues) {
    ...
    const result = await updateSubmission(submissionData, image);
    ...
    if (result?.error) {
        ...
        toast({
            ...
            description: "A sua sugestão foi atualizada com sucesso.",
        });
    }
  }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-headline text-3xl">Editar Submissão</CardTitle>
        <CardDescription>
          Ajuste os detalhes da sua descoberta. A sua contribuição é importante.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
                ...
                render={({ field }) => (
                    <FormItem>
                    <FormLabel>Link de Referência (Opcional)</FormLabel>
                    <FormControl>
                        <Input {...field} />
                    </FormControl>
                    <FormMessage />
                    ...
                    <FormItem>
                        ...
                        {submission.image_url && (
                            <div className="relative h-40 w-full max-w-sm rounded-md overflow-hidden border">
                                <Image src={submission.image_url} alt="Imagem atual da submissão" fill style={{ objectFit: 'cover' }} />
                            </div>
                        )}
                        <FormControl>
                            <Input 
