...
export function ConfrariaCard({ confraria }: ConfrariaCardProps) {
  return (
    <Link href={`/confrarias/${confraria.id}`} className="h-full">
        <Card className="flex flex-col h-full text-center items-center p-4 transition-transform transform hover:-translate-y-1 duration-300 hover:shadow-xl border-2 border-primary">
        <CardHeader>
        ...
        <CardFooter className="flex justify-center gap-4 w-full">
            <Badge variant="outline" className="flex items-center gap-1">
                ...
                {confraria.municipality}, {confraria.district}
            </Badge>
            <Badge variant="outline" className="flex items-center gap-1">
